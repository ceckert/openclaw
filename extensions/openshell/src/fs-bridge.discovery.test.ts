...
describe("openshell fs bridge discovery", () => {
  it("lists typed entries without traversing symbolic links", async () => {
  ...
  it("stops the listing for an aborted caller signal", async () => {
    await withWorkspaceBridge(async ({ bridge, workdir }) => {
  ...
  it("rejects directories over the discovery entry limit", async () => {
    await withWorkspaceBridge(async ({ bridge, workdir }) => {
