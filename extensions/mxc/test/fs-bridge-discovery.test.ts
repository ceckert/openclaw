...
describe("mxc fs bridge discovery", () => {
  test("lists typed entries without traversing symbolic links", async () => {
  ...
  test("stops the listing for an aborted caller signal", async () => {
    await withWorkspaceBridge(async ({ bridge, workdir }) => {
  ...
  test("rejects directories over the discovery entry limit", async () => {
    await withWorkspaceBridge(async ({ bridge, workdir }) => {
      ...
      for (let start = 0; start < names.length; start += 500) {
        await Promise.all(
          names.slice(start, start + 500).map((name) => fs.writeFile(path.join(workdir, name), "")),
        );
      }
