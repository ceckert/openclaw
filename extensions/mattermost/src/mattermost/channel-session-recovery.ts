...
type RecoveryPost = MattermostPost & { create_at: number };
...
const defaultDependencies: RecoveryDependencies = {
  ...
  sessionHasState: ({ cfg, agentId, sessionKey }) => {
    const storePath = resolveStorePath(cfg.session?.store, { agentId });
    const entry = getSessionEntry({ storePath, sessionKey, readConsistency: "latest" });
    return Boolean(
      entry &&
      (entry.archivedAt !== undefined ||
        entry.initializationPending ||
        readTranscriptStatsSync({ storePath, sessionKey, sessionId: entry.sessionId }).eventCount >
          0),
    );
  },
};
...
function hasCanonicalRestPostShape(post: MattermostPost): post is RecoveryPost {
...
function isPostAvailable(post: MattermostPost): post is RecoveryPost {
  return hasCanonicalRestPostShape(post) && post.delete_at === 0;
}

function isOrdinaryPost(post: MattermostPost): post is RecoveryPost {
...
function isHumanThreadPost(
  post: MattermostPost,
  params: { root: MattermostPost; channelId: string; botUserId: string },
): post is RecoveryPost {
  return (
    Boolean(normalized(post.id)) &&
    Boolean(normalized(post.user_id)) &&
    post.user_id !== params.botUserId &&
    post.channel_id === params.channelId &&
    (post.id === params.root.id
      ? !normalized(post.root_id)
      : normalized(post.root_id) === params.root.id) &&
    isOrdinaryPost(post) &&
...
function isCommittedAnswerPost(
  post: MattermostPost,
  params: {
  ...
  const postCreatedAt = finiteTimestamp(post.create_at);
  const postEditedAt = isZeroOrMissing(post.edit_at) ? 0 : finiteTimestamp(post.edit_at);
  return (
    ...
    post.id === params.part.postId &&
    post.id !== params.commit.post.id &&
    post.user_id === params.botUserId &&
    post.channel_id === params.channelId &&
    normalized(post.root_id) === params.commit.ref.mainRootPostId &&
    isOrdinaryPost(post) &&
    postBody(post) !== undefined &&
    hasMatchingOptionalRunRef(post, params.commit.ref)
  );
}
...
export function buildMattermostChannelRecoveryHistory(params: {
  ...
  const validCommits: Array<{ commit: RecoveryAnswerCommit; posts: RecoveryPost[] }> = [];
  for (const commit of parsedCommits.toSorted((left, right) =>
    ...
    if (
      ...
      !isHumanThreadPost(input, { root, ...params }) ||
      input.create_at > commit.post.create_at
    ) {
      continue;
    }
    const answerPosts: RecoveryPost[] = [];
    ...
    for (const part of commit.parts) {
      ...
      if (
        ...
        !isCommittedAnswerPost(post, {
          part,
          commit,
          channelId: params.channelId,
          botUserId: params.botUserId,
        }) ||
        post.create_at < input.create_at
      ) {
  ...
  const latestCommitByInput = new Map<
    string,
    { commit: RecoveryAnswerCommit; posts: RecoveryPost[] }
  >();
  for (const candidate of validCommits) {
  ...
  const history = roots.flatMap((root) => {
    ...
    for (const { commit, posts } of latestCommitByInput.values()) {
      ...
      const timestamp = Math.max(...posts.map((post) => post.create_at));
      if (!bodies) {
        continue;
      }
      timeline.push({
  ...
  const maxEntries = Math.max(0, Math.min(params.maxEntries, MAX_RECOVERY_ENTRIES));
  return maxEntries > 0
    ? history.slice(-maxEntries).map(({ sortId: _sortId, ...entry }) => entry)
    : [];
}
