...
type StoredAgentActivityRun = AgentActivitySnapshotRun & { primaryPostId?: string };
...
type StartRun = Omit<
  AgentActivitySnapshotRun,
  "activityChannelId" | "activityRootPostId" | "revision"
>;
...
export function createAgentActivityRuntime(options?: {
  ...
  const runs = new Map<string, StoredAgentActivityRun>();
  ...
  const startRun = (run: StartRun): StoredAgentActivityRun => {
    ...
    if (run.conversationId !== run.mainChannelId || run.turnId !== run.mainRootPostId) {
      throw new Error(`Agent activity run ${run.runId} must use its main conversation root`);
    }
    const stored: StoredAgentActivityRun = {
      ...run,
      activityChannelId: run.mainChannelId,
      activityRootPostId: run.mainRootPostId,
      revision: 1,
    };
  ...
  const clearRunPrimaryPost = (
    ...
    primaryPostOwners.delete(primaryPostId);
    runs.set(runId, {
      ...withoutPrimaryPost,
      revision: current.revision + 1,
    });
  ...
  const activeRunForConversation = (conversationId: string): StoredAgentActivityRun | undefined =>
    [...runs.values()]
