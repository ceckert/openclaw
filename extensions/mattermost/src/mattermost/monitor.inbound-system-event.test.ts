...
describe("mattermost inbound user posts", () => {
  ...
  it.each(["first", "all", "batched"] as const)(
    "threads and commits every answer part in %s mode",
    async (replyToMode) => {
      ...
      mockState.runtimeCore.channel.text.chunkMarkdownTextWithMode = vi.fn(() => [
        "Durable",
        "answer",
      ]);
      for (const [index, content] of ["Durable", "answer"].entries()) {
        const messageId = `answer-post-${index + 1}`;
      ...
      for (const call of mockState.sendMessageMattermost.mock.calls) {
        expect(call[2]).toEqual(expect.objectContaining({ replyToId: "input-post-1" }));
      }
      ...
      expect(events[1]).toEqual(
        expect.objectContaining({
          ...
          data: expect.objectContaining({
            ...
            postIds: ["answer-post-1", "answer-post-2"],
          }),
        }),
      );
    },
