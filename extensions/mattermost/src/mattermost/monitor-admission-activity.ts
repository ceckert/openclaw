// Mattermost plugin module wires durable run admission and live activity state.
...
export async function createMattermostAdmissionActivityWiring(params: {
  ...
  const { account, client, core, botUserId, runtime } = monitor;
  const services: {
  ...
  return { admissionService, activityRuntime, unregister };
}
