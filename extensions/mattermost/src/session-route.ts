...
export function resolveMattermostOutboundSessionRoute(params: ChannelOutboundSessionRouteParams) {
  ...
  const channelScoped =
    kind !== "direct" &&
    inspectMattermostAccount({ cfg: params.cfg, accountId: params.accountId }).config
      .threadSessionScope === "channel";
  return channelScoped
    ? { ...threadedRoute, sessionKey: threadedRoute.baseSessionKey, parentSessionKey: undefined }
    : threadedRoute;
}
