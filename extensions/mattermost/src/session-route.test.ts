...
describe("mattermost session route", () => {
  ...
  it("keeps the native thread but shares the channel session when configured", () => {
    const route = resolveMattermostOutboundSessionRoute({
      cfg: { channels: { mattermost: { threadSessionScope: "channel" } } },
      agentId: "main",
      accountId: "acct-1",
      target: "mattermost:channel:chan123",
      threadId: "thread456",
    ...
    const channelRoute = expectRoute(route);
    expect(channelRoute.threadId).toBe("thread456");
    expect(channelRoute.baseSessionKey).toBe("agent:main:mattermost:channel:chan123");
    expect(channelRoute.sessionKey).toBe(channelRoute.baseSessionKey);
  });

  it("keeps direct-message thread sessions isolated", () => {
    const route = resolveMattermostOutboundSessionRoute({
      cfg: {
      ...
      agentId: "main",
      accountId: "acct-1",
      target: "mattermost:user:user123",
      threadId: "thread456",
    });

    const directRoute = expectRoute(route);
    expect(directRoute.chatType).toBe("direct");
    expect(directRoute.threadId).toBe("thread456");
    expect(directRoute.sessionKey).not.toBe(directRoute.baseSessionKey);
  });
