...
| `group:openclaw`   | most built-in OpenClaw tools (excludes the `read`/`grep`/`find`/`ls`/`write`/`edit`/`apply_patch`/`exec`/`process` fs and runtime primitives, `canvas`, and provider plugins)                                                                            |
