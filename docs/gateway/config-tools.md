...
On Windows host sessions, unrestricted filesystem-only discovery uses bounded in-process operations. Workspace-confined discovery is registered only when helper processes are policy-authorized; sandboxed Windows sessions can also expose it through a backend `listDirectory` bridge. Filesystem-only local Windows sessions with `tools.fs.workspaceOnly: true` omit discovery because Node does not provide a safe descriptor-relative directory iterator.
