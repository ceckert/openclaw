...
<AccordionGroup>
  ...
  <Accordion title="Session control">
    ...
    - `sessions.reset`, `sessions.delete`, and `sessions.compact` perform session maintenance. `sessions.delete` archives an ordinary transcript by default. An `operator.admin` caller can set `deleteTranscriptWithoutArchive: true` to permanently remove transcript rows and receive an empty `archived` array; this irreversible option cannot be combined with `deleteTranscript: false`. Incognito transcript deletion remains archive-free. `archivedOnly: true` retains the narrower `operator.write` path for deleting an already archived session and does not authorize the archive-free option.
