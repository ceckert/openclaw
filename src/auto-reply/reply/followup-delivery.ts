...
async function sendFollowupPayloads(params: {
  ...
  const sourceObserver =
    params.kind === "final" && sourceDisposition?.kind === "observe"
      ? sourceDisposition.observer
      : undefined;
  const deliveryResults: OutboundDeliveryResult[] = [];
  const fallbackMessageIds: string[] = [];
  let finalDeliveryStarted = false;
  let finalDeliverySettled = false;
  let deliveryFailed = false;
  let deliverySuppressed = false;
  const observedResult = () => {
    const receipt = createMessageReceiptFromOutboundResults({ results: deliveryResults });
    const messageIds = [
      ...listMessageReceiptPlatformIds(receipt),
      ...fallbackMessageIds.filter(
        (messageId) => !listMessageReceiptPlatformIds(receipt).includes(messageId),
      ),
    ];
    return {
      ...(messageIds.length > 0 ? { messageIds } : {}),
      ...(receipt.parts.length > 0 ? { receipt } : {}),
      visibleReplySent: messageIds.length > 0,
      ...(deliverySuppressed ? { suppression: { reason: "queued_followup_suppressed" } } : {}),
    };
  };
  const startFinalDelivery = () => {
    if (!sourceObserver || finalDeliveryStarted) {
      return;
    }
    finalDeliveryStarted = true;
    sourceObserver.onFinalReplyStart();
  };
  const settleFinalDelivery = () => {
    if (!sourceObserver || finalDeliverySettled) {
      return;
    }
    finalDeliverySettled = true;
    const result = observedResult();
    if (deliveryFailed) {
      sourceObserver.onFinalReplyFailed(result);
    } else {
      sourceObserver.onFinalReplyDelivered(result);
    }
  };
  try {
    ...
    const deliverQueuedBatch =
      sourceDisposition?.kind === "deliver" ? sourceDisposition.deliver : undefined;
    ...
    for (const payload of payloads) {
      ...
      if (route !== "origin") {
        ...
        const result = await routeReply({
          ...
          ...(sourceObserver
            ? {
                onDeliveryResult: (deliveryResult: OutboundDeliveryResult) => {
                  deliveryResults.push(deliveryResult);
                },
              }
            : {}),
        });
        if (result.messageId && !fallbackMessageIds.includes(result.messageId)) {
          fallbackMessageIds.push(result.messageId);
        }
        deliveryFailed ||= !result.ok && !result.suppressed;
        deliverySuppressed ||= result.suppressed === true;
...
export async function deliverFollowupDecision(params: {
  ...
  const kind = params.kind ?? "final";
  const sourceDisposition = turn.queued.queuedFollowupReplyDisposition;
  const sourceObserver =
    kind === "final" && sourceDisposition?.kind === "observe"
      ? sourceDisposition.observer
      : undefined;
  const send = async (sendParams: Parameters<typeof sendFollowupPayloads>[0]) => {
    try {
      await sendFollowupPayloads(sendParams);
    } finally {
      sourceObserver?.onDispatcherSettled();
    }
  };
  if (decision.kind === "suppress") {
    logVerbose(`followup queue: delivery suppressed (${decision.reason})`);
    sourceObserver?.onFinalReplyDelivered({
      visibleReplySent: false,
      suppression: { reason: decision.reason },
    });
    sourceObserver?.onDispatcherSettled();
    return;
  }
