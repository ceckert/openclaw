...
function resolvePinnedDirectoryPath(stats: Stats, fd: number): string {
  if (process.platform === "linux") {
    return `/proc/self/fd/${fd}`;
  }
  if (process.platform === "darwin") {
    return `/.vol/${stats.dev}/${stats.ino}`;
  }
  ...
  return {
    async *entries(relativePath, options) {
      ...
      try {
        ...
        const pinnedPath = resolvePinnedDirectoryPath(stats, handle.fd);
