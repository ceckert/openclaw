...
it("does not advertise confined filesystem-only host discovery without a safe Windows iterator", () => {
  const tools = createWindowsDiscoveryTools({
...
it("keeps helper-authorized Windows host discovery registered", () => {
  const tools = createWindowsDiscoveryTools({
