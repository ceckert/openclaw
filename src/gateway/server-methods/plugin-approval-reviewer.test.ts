...
function fixture(test: TestContext) {
  ...
  return {
    ...
    async resolve(
      ...
      const respond = vi.fn();
      await handlers[method]!({
        req: { id: "resolve-1", type: "req", method, params },
        params,
        context,
        client: {
...
describe.each(["approval.resolve", "plugin.approval.resolve"] as const)(
  ...
  (method) => {
    ...
    it("rechecks reviewer authority at the native worker mutation boundary", async (test) => {
      ...
      using dispatched = vi
        ...
        .mockImplementation((input) => {
          expect(input.guard?.assertCurrent).toEqual(expect.any(Function));
...
describe("in-process plugin approval reviewer request", () => {
  it("hides resolution failures after native reviewer access is revoked", async (test) => {
    const { manager, resolve } = fixture(test);
    const scopes = ["operator.admin"];
    const record = manager.create(
      { title: "Install editor", description: "Shared computer" },
      60_000,
      "plugin:post-error-revocation",
    );
    await manager.register(record, 60_000);
    using failed = vi.spyOn(manager, "resolveDetailed").mockImplementation(async () => {
      scopes.length = 0;
      throw new Error("private storage diagnostic");
    });
    const response = await resolve("approval.resolve", record.id, "owner", scopes);
