...
export async function handleGatewayRequest(
  ...
  const rejectRateLimitedControlPlaneWrite = (): boolean => {
    ...
    // reload + config.patch post-write-hash (no-op skip) patches. Every untrusted
    // caller (device-token, anonymous) and every other control-plane write still
    // gets the 3/60s limit.
    if (req.method === "config.patch" && client?.usesSharedGatewayAuth === true) {
      return false;
    }
