...
describe("gateway host desktop source", () => {
  ...
  it("resolves a redacted password-file value separately from the loopback attachment", async () => {
    ...
    const root = tempDirs.make("openclaw-host-desktop-");
  ...
  it.each([false, true])(
    ...
    async (managedConfigured) => {
      ...
      const service = createHostDesktopService({
        getConfig: () => ({ enabled: true, managed: managedConfigured, port, passwordFile }),
        platform: "linux",
        registry,
  ...
  it("keeps a managed desktop's password and active computer when observing again", async () => {
    ...
    const managed = fakeManagedDesktop();
    const service = createHostDesktopService({
      getConfig: () => ({ enabled: true, managed: true, passwordFile }),
      platform: "linux",
      registry,
      managedDesktop: managed.managed,
    });
  ...
  it.each(["stop", "revoke", "disable", "reconfigure"] as const)(
    ...
    async (invalidation) => {
      ...
      const registry = createDesktopSessionRegistry();
      let config: DesktopHostConfig = { enabled: true, port: 5901, passwordFile };
      const service = createHostDesktopService({
        getConfig: () => config,
        registry,
      });
      ...
      expect(registry.hasActivity("host", 0)).toBe(true);
      if (invalidation === "stop") {
        ...
        config =
          invalidation === "disable" ? { ...config, enabled: false } : { ...config, port: 5902 };
      }
      ...
      await expect(observing).rejects.toThrow(
        invalidation === "stop"
          ...
          : invalidation === "revoke"
            ...
            : "gateway host desktop configuration changed",
      );
  ...
  it("reports a missing password file on reconnect without reusing the previous credential", async () => {
    ...
    const registry = createDesktopSessionRegistry();
    const service = createHostDesktopService({
      getConfig: () => ({ enabled: true, port: 5901, passwordFile }),
      registry,
    });
    cleanups.push(() => registry.stopAll());
