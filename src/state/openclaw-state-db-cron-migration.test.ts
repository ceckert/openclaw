...
const reader = vi.hoisted(() => ({ supportedVersion: 19 }));
...
it("upgrades schema 18 without losing jobs and fences schema 18 readers from held migration state", () => {
  ...
  expect(upgraded.db.prepare("PRAGMA user_version").get()).toEqual({ user_version: 19 });
  ...
  reader.supportedVersion = 18;
  expect(() => assertSupportedStateSchemaVersion(upgraded.db, upgraded.path)).toThrow(
    /uses newer schema version 19; this build supports 18/,
  );
